...
@section('content')
    <div class="card-header text-center  mt-2">
        <h3 class="card-title">{{__('messages.Update Student')}}</h3>
    </div>
    <div class=" p-3 card card-primary mb-0">


        <form  class="form" method="post" action="{{route('users.update',$student->id)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
        ...
        <div class="mb-3 form-group">
            ...
            <input type="text" class="form-control" id="formUN" name="username" value="{{$student->username}}">
            <x-input-error :messages="$errors->get('username')" class="mt-2" />

        </div>
        <div class="mb-3">
            <label for="formFN" class="form-label">{{__('messages.firstName')}}</label>
            <input type="text" class="form-control" id="formFN"  name="firstName" value="{{$student->firstName}}">
            <x-input-error :messages="$errors->get('firstName')" class="mt-2" />

        </div>
        <div class="mb-3">
            <label for="formUN" class="form-label">{{__('messages.lastName')}}</label>
            <input type="text" class="form-control" id="formUN" name="lastName" value="{{$student->lastName}}">
        ...
        <div class="mb-3">
            ...
            <x-input-error :messages="$errors->get('password')" class="mt-2" />
                ...
                <div class="col-lg-6">
                    <div class="mb-3 col-12">
                        ...
                        <input type="text" class="form-control" id="formUN" name="email" value="{{$student->email}}">
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />

                    </div>
                    <div class="mb-3 col-12 ">
                        <label for="formFN" class="form-label">{{__('messages.address')}}</label>
                        <input type="text" class="form-control" id="formFN" name="address" value="{{$student->address}}">
            ...
            <div class="d-flex justify-content-center align-items-center ">
                <button type="submit" class="btn btn-primary w-50 mt-3 p-3 " > {{__('messages.Edit')}}</button>
